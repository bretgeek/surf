...
## An Insanely Small Plugin Extendable, Component Based Reactive Element Template Library
...
First it's an insanely small ~13kb plugin extendable JavaScript library for building user interfaces (UI). 
...
**Surf JS** has a really powerful feature called **"Reactive Element Templates"** that allows you to manipulate your HTML with changing data defined by **{% raw %} {{templates}} {% endraw %}**. Since Reactive Templates resolve to functions (once they are activated) there is no DOM querying...changes are live! 
